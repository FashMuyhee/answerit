...
class QuizService {
  ...
  fetchQuizzes = async () => {
    try {
      ...
      const ref = await firestore().collection('quizzes').get();
      ...
      const refScore = await firestore()
        .collection('score_board')
        .where('uid', '==', user.uid)
        .get();
      const myScores = refScore.docs.map(item => item.data());
      const exist = this.saveUnAnswered(data,myScores);
  ...
  fetchQuizDetail = async id => {
    try {
      const ref = await firestore().collection('quizzes').doc(id).get();
  ...
  submitScore = async ({ quizId, score, quizTitle, matricNo }) => {
    const user = auth().currentUser;
    try {
      const ref = await firestore().collection('score_board').add({
        quizId,
        score,
        createdAt: moment.now(),
        uid: user.uid,
